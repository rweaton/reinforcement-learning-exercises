function OutputStruct = ActorCriticSimulator2(StateVectors, ActionsList, nIterations, params)
    ...
    CriticWeightsArray = NaN*ones([nStates, nElements, nIterations]);
    ActionMatrixArray = NaN*ones([nActions,nElements, nIterations]);
    
    ExpectedRewards(1) = CriticWeights(StateIndices(1),:)*StateVectors(:,StateIndices(1));
    
    for i = 1:(nIterations-1)
        
        % Recompute ExpectedRewards after CriticWeights has been updated
        ExpectedRewards(i) = CriticWeights(StateIndices(i),:)*StateVectors(:,StateIndices(i));
        ...
        NewStateStruct = StateEvaluator(StateVectors(:,StateIndices(i)), Choice);
        ...
        % previous iteration
        if (EndOfTrial == 0)
            ExpectedRewards(i+1) = CriticWeights(StateIndices(i+1),:)*StateVectors(:,StateIndices(i+1));
        elseif (EndOfTrial == 1)
        ...
        CriticWeights(StateIndices(i),:) = CriticWeights(StateIndices(i),:)...
    ...
    AverageCriticWeights = sum(CriticWeightsArray(:,:,50:(nIterations-1)),3)/(nIterations - 50);
    
    figure; hold on;
    subplot(3,1,1), plot(squeeze(CriticWeightsArray(1,1,:)),'k'), hold on, ...
        plot(AverageCriticWeights(1,1)*ones([1,nIterations]),'r--');
    
    subplot(3,1,2), plot(squeeze(CriticWeightsArray(2,2,:)),'k'), hold on, ... 
    plot(AverageCriticWeights(2,2)*ones([1,nIterations]),'r--');
    
    subplot(3,1,3), plot(squeeze(CriticWeightsArray(3,3,:)),'k'), hold on, ... 
